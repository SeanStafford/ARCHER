...
Examples:
    ...
    python scripts/apply_presets.py apply resume.yaml colors_anthropic

    # Mix tight spacing with big bottom bar
    python scripts/apply_presets.py apply resume.yaml spacing_tight spacing_bigbottom

    # Apply multiple presets (colors + spacing)
    python scripts/apply_presets.py apply resume.yaml spacing_deluxe colors_netflix

    # Specify custom output path
    python scripts/apply_presets.py apply resume.yaml colors_anthropic -o resume_anthropic.yaml
"""
...
def print_command(
    preset_name: Annotated[
        str,
        typer.Argument(help="Preset name (e.g., 'colors_anthropic', 'spacing_tight')"),
    ],
):
    """
    ...
    Examples:\n
        $ apply_presets.py print colors_anthropic
...
def apply_command(
    yaml_path: Annotated[
        Path,
        typer.Argument(help="Path to resume YAML file", exists=True),
    ],
    presets: Annotated[
        list[str],
        typer.Argument(help="Preset names to apply (e.g., spacing_tight colors_anthropic)"),
    ],
    output: Annotated[
        Path,
        typer.Option(
    ...
    Examples:\n
        ...
        $ apply_presets.py apply resume.yaml colors_anthropic

        # Mix spacing presets
        $ apply_presets.py apply resume.yaml spacing_deluxe spacing_bigbottom

        # Multiple presets with custom output
        $ apply_presets.py apply resume.yaml spacing_tight colors_netflix -o resume_variant.yaml
    """
...
if __name__ == "__main__":
    ...
    # This allows: python apply_presets.py resume.yaml colors_anthropic
    # instead of:  python apply_presets.py apply resume.yaml colors_anthropic
