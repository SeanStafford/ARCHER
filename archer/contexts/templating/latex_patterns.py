...
def regex_to_literal(regex_pattern: str) -> str:
    ...
    return (regex_pattern
            .replace('\\\\', '\\')   # Double backslash -> single backslash
            .replace('\\{', '{')      # Escaped brace -> literal brace
            .replace('\\}', '}')      # Escaped brace -> literal brace
            .replace('\\*', '*'))     # Escaped asterisk -> literal asterisk
...
class PageRegex:
    ...
    PAGE_SETUP_AFTER_CLEARPAGE: str = r'\\clearpage.*?(\\toggletrue\{useleftbar\}.*?\\pagestyle\{resume\})'

    # Decorations following textblock (leftgrad/bottombar commands)
    DECORATIONS_FOLLOWING_TEXTBLOCK: str = r'(\s*\\(?:leftgrad|bottombar)\{[^}]*\}(?:\{[^}]*\})*\s*)*'
...
class SectionRegex:
    ...
    SECTION_WITH_NAME: str = r'\\section\*?\{'  # Finds section start, name extracted with balanced braces
...
class EnvironmentPatterns:
    ...
    ITEMIZE_PROJECT_ENV: str = r'itemize.*Proj(?:ect|Second)'  # Env name pattern for extract_all_environments
    ...
    ITEMIZE_PROJ_SECOND_ENV: str = r'itemizeProjSecond'  # Env name pattern for extract_all_environments
    ...
    SKILL_CATEGORY_HEADER: str = r'\\item\[.*?\][^\n]*(?=\n[^\n]*\\addcontentsline\{toc\}\{section\})'
...
class MetadataRegex:
    ...
    LIST_TITLE_AFTER_NAME: str = r'\\toggle(true|false)\{list_title_after_name\}'  # Captures true/false
